...
st.write("This program will load an extended m3u playlist and display it using streamlit")
st.write("It will sort the contents by language and movies and tv series")
st.write("After user selects a movie it can download the movie to a file")
...
filtered_media = sorted([
    ...
    if (selected_group == "All" or item.group == selected_group) and
       ...
       (selected_language == "All" or item.lang == selected_language) and
       (search_title in item.title)
])
...
if not filtered_media:
    ...
    filtered_media_df = pd.DataFrame([
        {"Title": item.original_title, "Group": item.group, "Language": item.lang, 
